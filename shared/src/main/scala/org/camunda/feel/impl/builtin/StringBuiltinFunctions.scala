...
object StringBuiltinFunctions {
  ...
  private def substringFunction3 = builtinFunction(
    ...
    invoke = { case List(ValString(string), ValNumber(start), ValNumber(length)) =>
      ValString(
        string.substring(stringIndex(string, start.intValue), stringIndex(string, start.intValue) + length.intValue)
      )
    }
  )
  ...
  private def replaceFunction = builtinFunction(
    ...
    invoke = { case List(ValString(input), ValString(pattern), ValString(replacement)) =>
      ...
      val matches = pattern.r.unanchored.unapplySeq(input).toSeq.flatten.filterNot(_ == null)
      val replaced =
        replacementRegex.replaceAllIn(replacement, m => matches.applyOrElse(m.group(1).toInt - 1, (_: Int) => ""))
