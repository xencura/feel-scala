...
import org.camunda.feel.{syntaxtree, Date, DateTime, DayTimeDuration, Time, YearMonthDuration}
...
class DefaultValueMapper extends CustomValueMapper {
  ...
  def toVal(x: Any, innerValueMapper: Any => Val): Option[Val] = x match {
    ...
    case x: java.util.Date =>
      Some(ValLocalDateTime(LocalDateTime.ofInstant(x.toInstant, ZoneId.systemDefault())))
    ...
    case x: java.util.Map[_, _] =>
      Some(ValContext(Context.StaticContext(x.asScala.map { case (key, value) =>
        key.toString -> innerValueMapper(value)
      }.toMap)))
  ...
  def unpackVal(value: Val, innerValueMapper: Val => Any): Option[Any] =
    value match {
      ...
      case ValContext(c: Context) =>
        Some(c.variableProvider.getVariables.map { case (key, value) =>
          value match {
